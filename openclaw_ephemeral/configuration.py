...
def _embedding_config(environ: Mapping[str, str]) -> tuple[dict[str, Any], dict[str, Any]]:
    """Configure explicit embedding endpoints without discovering chat models."""
    ...
    for index in range(1, 51):
        ...
        values = {
            field: clean(environ.get(f"{prefix}{field}{suffix}"))
...
def build_config(
    ...
    *,
    ...
    if embedding_providers:
        ...
        collisions = model_providers.keys() & embedding_providers.keys()
        if collisions:
            raise ConfigurationError(
                "Embedding provider names must differ from chat providers: "
